<!DOCTYPE html>
<html lang="en">
<head>
<%@ include file="../../head.jsp"%>
<script type="text/javascript" src="../../js/pagination.js"></script>
<link href="../../css/project.css" rel="stylesheet" />
</head>
<body>

	<!-- DO NOT EDIT THIS LINE AND ANY LINES BETWEEN THEM -->
	<!-- NAVAIGATION#02 -->
	<%@ include file="../../nav.jsp"%>
	<!-- END OF NAVAIGATION#02 -->
	<div id="content">
		<!-- INSERT YOUR CODE HERE#00-->
		<!--项目列表-->
		<div class="container">
			<div class="row clearfix">
				<div class="col-md-9 column">
					<input type='hidden' id='currentPage' /> <input type='hidden'
						id='numPerPage' />
					<ul id="project-content">
						<li class="project-tag">
							<div class="tag-box">

								<!-- 主要信息 -->
								<div class="project-box">
									<a href="projectview.jsp" class="project-drb"> <img
										class="tag-image" src="../../img/background-img.jpg">
										<h3>创新创业大赛</h3>
										<div class="project-text">
											<p>描述：华师大计软学院举办的创新创业大赛，做一个项目发布的网站</p>
										</div>
									</a> <a href="#"><span class="require label label-default">C/C++</span></a>
									<a href="#"><span class="require label label-primary">Java</span></a>
								</div>

								<!-- 发布信息 -->
								<div class="creator-box">
									<div style="display: inline text-align:center">
										<img src="../../img/avater.png" class="img-circle"
											id="img-down"> <a
											href="../person-view/personal-view.html" id="person-info">机制汇</a>

										<img src="../../img/person.png"
											style="width: 20px; height: 20px; margin-left: 20px"
											title="人数">10
									</div>
									<h6>2015-11-3</h6>
								</div>
							</div>
						</li>

						<li class="project-tag">
							<div class="tag-box">

								<!-- 主要信息 -->
								<div class="project-box">
									<a href="projectview.jsp" class="project-drb"> <img
										class="tag-image" src="../../img/background-img.jpg">
										<h3>创新创业大赛</h3>
										<div class="project-text">
											<p>描述：华师大计软学院举办的创新创业大赛，做一个项目发布的网站</p>
										</div>
									</a> <a href="#"><span class="require label label-default">C/C++</span></a>
									<a href="#"><span class="require label label-primary">Java</span></a>
								</div>

								<!-- 发布信息 -->
								<div class="creator-box">
									<div style="display: inline text-align:center">
										<img src="../../img/avater.png" class="img-circle"
											id="img-down"> <a
											href="../person-view/personal-view.html" id="person-info">机制汇</a>

										<img src="../../img/person.png"
											style="width: 20px; height: 20px; margin-left: 20px"
											title="人数">10
									</div>
									<h6>2015-11-3</h6>
								</div>
							</div>
						</li>

						<li class="project-tag">
							<div class="tag-box">

								<!-- 主要信息 -->
								<div class="project-box">
									<a href="projectview.jsp" class="project-drb"> <img
										class="tag-image" src="../../img/background-img.jpg">
										<h3>创新创业大赛</h3>
										<div class="project-text">
											<p>描述：华师大计软学院举办的创新创业大赛，做一个项目发布的网站</p>
										</div>
									</a> <a href="#"><span class="require label label-default">C/C++</span></a>
									<a href="#"><span class="require label label-primary">Java</span></a>
								</div>

								<!-- 发布信息 -->
								<div class="creator-box">
									<div style="display: inline text-align:center">
										<img src="../../img/avater.png" class="img-circle"
											id="img-down"> <a
											href="../person-view/personal-view.html" id="person-info">机制汇</a>

										<img src="../../img/person.png"
											style="width: 20px; height: 20px; margin-left: 20px"
											title="人数">10
									</div>
									<h6>2015-11-3</h6>
								</div>
							</div>
						</li>

						<li class="project-tag">
							<div class="tag-box">

								<!-- 主要信息 -->
								<div class="project-box">
									<a href="projectview.jsp" class="project-drb"> <img
										class="tag-image" src="../../img/background-img.jpg">
										<h3>创新创业大赛</h3>
										<div class="project-text">
											<p>描述：华师大计软学院举办的创新创业大赛，做一个项目发布的网站</p>
										</div>
									</a> <a href="#"><span class="require label label-default">C/C++</span></a>
									<a href="#"><span class="require label label-primary">Java</span></a>
								</div>

								<!-- 发布信息 -->
								<div class="creator-box">
									<div style="display: inline text-align:center">
										<img src="../../img/avater.png" class="img-circle"
											id="img-down"> <a
											href="../person-view/personal-view.html" id="person-info">机制汇</a>

										<img src="../../img/person.png"
											style="width: 20px; height: 20px; margin-left: 20px"
											title="人数">10
									</div>
									<h6>2015-11-3</h6>
								</div>
							</div>
						</li>

						<li class="project-tag">
							<div class="tag-box">

								<!-- 主要信息 -->
								<div class="project-box">
									<a href="projectview.jsp" class="project-drb"> <img
										class="tag-image" src="../../img/background-img.jpg">
										<h3>创新创业大赛</h3>
										<div class="project-text">
											<p>描述：华师大计软学院举办的创新创业大赛，做一个项目发布的网站</p>
										</div>
									</a> <a href="#"><span class="require label label-default">C/C++</span></a>
									<a href="#"><span class="require label label-primary">Java</span></a>
								</div>

								<!-- 发布信息 -->
								<div class="creator-box">
									<div style="display: inline text-align:center">
										<img src="../../img/avater.png" class="img-circle"
											id="img-down"> <a
											href="../person-view/personal-view.html" id="person-info">机制汇</a>

										<img src="../../img/person.png"
											style="width: 20px; height: 20px; margin-left: 20px"
											title="人数">10
									</div>
									<h6>2015-11-3</h6>
								</div>
							</div>
						</li>

						<li class="project-tag">
							<div class="tag-box">

								<!-- 主要信息 -->
								<div class="project-box">
									<a href="projectview.jsp" class="project-drb"> <img
										class="tag-image" src="../../img/background-img.jpg">
										<h3>创新创业大赛</h3>
										<div class="project-text">
											<p>描述：华师大计软学院举办的创新创业大赛，做一个项目发布的网站</p>
										</div>
									</a> <a href="#"><span class="require label label-default">C/C++</span></a>
									<a href="#"><span class="require label label-primary">Java</span></a>
								</div>

								<!-- 发布信息 -->
								<div class="creator-box">
									<div style="display: inline text-align:center">
										<img src="../../img/avater.png" class="img-circle"
											id="img-down"> <a
											href="../person-view/personal-view.html" id="person-info">机制汇</a>

										<img src="../../img/person.png"
											style="width: 20px; height: 20px; margin-left: 20px"
											title="人数">10
									</div>
									<h6>2015-11-3</h6>
								</div>
							</div>
						</li>

						<li class="project-tag">
							<div class="tag-box">

								<!-- 主要信息 -->
								<div class="project-box">
									<a href="projectview.jsp" class="project-drb"> <img
										class="tag-image" src="../../img/background-img.jpg">
										<h3>创新创业大赛</h3>
										<div class="project-text">
											<p>描述：华师大计软学院举办的创新创业大赛，做一个项目发布的网站</p>
										</div>
									</a> <a href="#"><span class="require label label-default">C/C++</span></a>
									<a href="#"><span class="require label label-primary">Java</span></a>
								</div>

								<!-- 发布信息 -->
								<div class="creator-box">
									<div style="display: inline text-align:center">
										<img src="../../img/avater.png" class="img-circle"
											id="img-down"> <a
											href="../person-view/personal-view.html" id="person-info">机制汇</a>

										<img src="../../img/person.png"
											style="width: 20px; height: 20px; margin-left: 20px"
											title="人数">10
									</div>
									<h6>2015-11-3</h6>
								</div>
							</div>
						</li>

						<li class="project-tag">
							<div class="tag-box">

								<!-- 主要信息 -->
								<div class="project-box">
									<a href="projectview.jsp" class="project-drb"> <img
										class="tag-image" src="../../img/background-img.jpg">
										<h3>创新创业大赛</h3>
										<div class="project-text">
											<p>描述：华师大计软学院举办的创新创业大赛，做一个项目发布的网站</p>
										</div>
									</a> <a href="#"><span class="require label label-default">C/C++</span></a>
									<a href="#"><span class="require label label-primary">Java</span></a>
								</div>

								<!-- 发布信息 -->
								<div class="creator-box">
									<div style="display: inline text-align:center">
										<img src="../../img/avater.png" class="img-circle"
											id="img-down"> <a
											href="../person-view/personal-view.html" id="person-info">机制汇</a>

										<img src="../../img/person.png"
											style="width: 20px; height: 20px; margin-left: 20px"
											title="人数">10
									</div>
									<h6>2015-11-3</h6>
								</div>
							</div>
						</li>

						<li class="project-tag">
							<div class="tag-box">

								<!-- 主要信息 -->
								<div class="project-box">
									<a href="projectview.jsp" class="project-drb"> <img
										class="tag-image" src="../../img/background-img.jpg">
										<h3>创新创业大赛</h3>
										<div class="project-text">
											<p>描述：华师大计软学院举办的创新创业大赛，做一个项目发布的网站</p>
										</div>
									</a> <a href="#"><span class="require label label-default">C/C++</span></a>
									<a href="#"><span class="require label label-primary">Java</span></a>
								</div>

								<!-- 发布信息 -->
								<div class="creator-box">
									<div style="display: inline text-align:center">
										<img src="../../img/avater.png" class="img-circle"
											id="img-down"> <a
											href="../person-view/personal-view.html" id="person-info">机制汇</a>

										<img src="../../img/person.png"
											style="width: 20px; height: 20px; margin-left: 20px"
											title="人数">10
									</div>
									<h6>2015-11-3</h6>
								</div>

							</div>
						</li>

						<li class="project-tag">
							<div class="tag-box">

								<!-- 主要信息 -->
								<div class="project-box">
									<a href="projectview.jsp" class="project-drb"> <img
										class="tag-image" src="../../img/background-img.jpg">
										<h3>创新创业大赛</h3>
										<div class="project-text">
											<p>描述：华师大计软学院举办的创新创业大赛，做一个项目发布的网站</p>
										</div>
									</a> <a href="#"><span class="require label label-default">C/C++</span></a>
									<a href="#"><span class="require label label-primary">Java</span></a>
								</div>

								<!-- 发布信息 -->
								<div class="creator-box">
									<div style="display: inline text-align:center">
										<img src="../../img/avater.png" class="img-circle"
											id="img-down"> <a
											href="../person-view/personal-view.html" id="person-info">机制汇</a>

										<img src="../../img/person.png"
											style="width: 20px; height: 20px; margin-left: 20px"
											title="人数">10
									</div>
									<h6>2015-11-3</h6>
								</div>

							</div>
						</li>

						<li class="project-tag">
							<div class="tag-box">

								<!-- 主要信息 -->
								<div class="project-box">
									<a href="projectview.jsp" class="project-drb"> <img
										class="tag-image" src="../../img/background-img.jpg">
										<h3>创新创业大赛</h3>
										<div class="project-text">
											<p>描述：华师大计软学院举办的创新创业大赛，做一个项目发布的网站</p>
										</div>
									</a> <a href="#"><span class="require label label-default">C/C++</span></a>
									<a href="#"><span class="require label label-primary">Java</span></a>
								</div>

								<!-- 发布信息 -->
								<div class="creator-box">
									<div style="display: inline text-align:center">
										<img src="../../img/avater.png" class="img-circle"
											id="img-down"> <a
											href="../person-view/personal-view.html" id="person-info">机制汇</a>

										<img src="../../img/person.png"
											style="width: 20px; height: 20px; margin-left: 20px"
											title="人数">10
									</div>
									<h6>2015-11-3</h6>
								</div>

							</div>
						</li>

						<li class="project-tag">
							<div class="tag-box">

								<!-- 主要信息 -->
								<div class="project-box">
									<a href="projectview.jsp" class="project-drb"> <img
										class="tag-image" src="../../img/background-img.jpg">
										<h3>创新创业大赛</h3>
										<div class="project-text">
											<p>描述：华师大计软学院举办的创新创业大赛，做一个项目发布的网站</p>
										</div>
									</a> <a href="#"><span class="require label label-default">C/C++</span></a>
									<a href="#"><span class="require label label-primary">Java</span></a>
								</div>

								<!-- 发布信息 -->
								<div class="creator-box">
									<div style="display: inline text-align:center">
										<img src="../../img/avater.png" class="img-circle"
											id="img-down"> <a
											href="../person-view/personal-view.html" id="person-info">机制汇</a>

										<img src="../../img/person.png"
											style="width: 20px; height: 20px; margin-left: 20px"
											title="人数">10
									</div>
									<h6>2015-11-3</h6>
								</div>

							</div>
						</li>

						<li class="project-tag">
							<div class="tag-box">

								<!-- 主要信息 -->
								<div class="project-box">
									<a href="projectview.jsp" class="project-drb"> <img
										class="tag-image" src="../../img/background-img.jpg">
										<h3>创新创业大赛</h3>
										<div class="project-text">
											<p>描述：华师大计软学院举办的创新创业大赛，做一个项目发布的网站</p>
										</div>
									</a> <a href="#"><span class="require label label-default">C/C++</span></a>
									<a href="#"><span class="require label label-primary">Java</span></a>
								</div>

								<!-- 发布信息 -->
								<div class="creator-box">
									<div style="display: inline text-align:center">
										<img src="../../img/avater.png" class="img-circle"
											id="img-down"> <a
											href="../person-view/personal-view.html" id="person-info">机制汇</a>

										<img src="../../img/person.png"
											style="width: 20px; height: 20px; margin-left: 20px"
											title="人数">10
									</div>
									<h6>2015-11-3</h6>
								</div>

							</div>
						</li>

						<li class="project-tag">
							<div class="tag-box">

								<!-- 主要信息 -->
								<div class="project-box">
									<a href="projectview.jsp" class="project-drb"> <img
										class="tag-image" src="../../img/background-img.jpg">
										<h3>创新创业大赛</h3>
										<div class="project-text">
											<p>描述：华师大计软学院举办的创新创业大赛，做一个项目发布的网站</p>
										</div>
									</a> <a href="#"><span class="require label label-default">C/C++</span></a>
									<a href="#"><span class="require label label-primary">Java</span></a>
								</div>

								<!-- 发布信息 -->
								<div class="creator-box">
									<div style="display: inline text-align:center">
										<img src="../../img/avater.png" class="img-circle"
											id="img-down"> <a
											href="../person-view/personal-view.html" id="person-info">机制汇</a>

										<img src="../../img/person.png"
											style="width: 20px; height: 20px; margin-left: 20px"
											title="人数">10
									</div>
									<h6>2015-11-3</h6>
								</div>

							</div>
						</li>

						<li class="project-tag">
							<div class="tag-box">

								<!-- 主要信息 -->
								<div class="project-box">
									<a href="projectview.jsp" class="project-drb"> <img
										class="tag-image" src="../../img/background-img.jpg">
										<h3>创新创业大赛</h3>
										<div class="project-text">
											<p>描述：华师大计软学院举办的创新创业大赛，做一个项目发布的网站</p>
										</div>
									</a> <a href="#"><span class="require label label-default">C/C++</span></a>
									<a href="#"><span class="require label label-primary">Java</span></a>
								</div>

								<!-- 发布信息 -->
								<div class="creator-box">
									<div style="display: inline text-align:center">
										<img src="../../img/avater.png" class="img-circle"
											id="img-down"> <a
											href="../person-view/personal-view.html" id="person-info">机制汇</a>

										<img src="../../img/person.png"
											style="width: 20px; height: 20px; margin-left: 20px"
											title="人数">10
									</div>
									<h6>2015-11-3</h6>
								</div>

							</div>
						</li>

						<li class="project-tag">
							<div class="tag-box">

								<!-- 主要信息 -->
								<div class="project-box">
									<a href="projectview.jsp" class="project-drb"> <img
										class="tag-image" src="../../img/background-img.jpg">
										<h3>创新创业大赛</h3>
										<div class="project-text">
											<p>描述：华师大计软学院举办的创新创业大赛，做一个项目发布的网站</p>
										</div>
									</a> <a href="#"><span class="require label label-default">C/C++</span></a>
									<a href="#"><span class="require label label-primary">Java</span></a>
								</div>

								<!-- 发布信息 -->
								<div class="creator-box">
									<div style="display: inline text-align:center">
										<img src="../../img/avater.png" class="img-circle"
											id="img-down"> <a
											href="../person-view/personal-view.html" id="person-info">机制汇</a>

										<img src="../../img/person.png"
											style="width: 20px; height: 20px; margin-left: 20px"
											title="人数">10
									</div>
									<h6>2015-11-3</h6>
								</div>

							</div>
						</li>

						<li class="project-tag">
							<div class="tag-box">

								<!-- 主要信息 -->
								<div class="project-box">
									<a href="projectview.jsp" class="project-drb"> <img
										class="tag-image" src="../../img/background-img.jpg">
										<h3>创新创业大赛</h3>
										<div class="project-text">
											<p>描述：华师大计软学院举办的创新创业大赛，做一个项目发布的网站</p>
										</div>
									</a> <a href="#"><span class="require label label-default">C/C++</span></a>
									<a href="#"><span class="require label label-primary">Java</span></a>
								</div>

								<!-- 发布信息 -->
								<div class="creator-box">
									<div style="display: inline text-align:center">
										<img src="../../img/avater.png" class="img-circle"
											id="img-down"> <a
											href="../person-view/personal-view.html" id="person-info">机制汇</a>

										<img src="../../img/person.png"
											style="width: 20px; height: 20px; margin-left: 20px"
											title="人数">10
									</div>
									<h6>2015-11-3</h6>
								</div>

							</div>
						</li>
					</ul>
				</div>


				<div class="col-md-3 column">
					<div class="list-group"
						style="position: fixed; float: right; width: 200px; margin-top: 20px">
						<a href="#" class="list-group-item active"><span class="badge">60</span>所有项目</a>
						<a href="#" class="list-group-item"><span class="badge">20</span>C/C++</a>
						<a href="#" class="list-group-item"><span class="badge">20</span>Java</a>
						<a href="#" class="list-group-item"><span class="badge">40</span>老师发布</a>
						<a href="#" class="list-group-item"><span class="badge">20</span>学生发布</a>
					</div>
				</div>
			</div>
		</div>
		<div class="row" style="margin: 30px">
			<div class="col-md-4 column"></div>
			<div class="col-md-4 column">
				<ul class="pagination" id="paging">
				</ul>
			</div>
		</div>
	</div>

	<!-- END OF YOUR CODE#00 -->


	<!-- DO NOT EDIT THIS LINE AND ANY LINES BETWEEN THEM -->
	<!-- BOTTOM#03 -->
	<%@ include file="../../bottom.jsp"%>
	<!-- END OF BOTTOM#03 -->
</body>
</html>
